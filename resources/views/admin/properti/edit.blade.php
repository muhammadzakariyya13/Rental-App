        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.properti.update', $properti) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="kamar_tidur" name="kamar_tidur" value="{{ $properti->kamar_tidur ?? old('kamar_tidur') }}" min="0" class="w-full px-4 py-2 border dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded {{ $errors->has('kamar_tidur') ? 'border-red-500' : 'border-gray-300' }}">
                                @error('kamar_tidur')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <input type="number" id="kamar_mandi" name="kamar_mandi" value="{{ $properti->kamar_mandi ?? old('kamar_mandi') }}" min="0" class="w-full px-4 py-2 border dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded {{ $errors->has('kamar_mandi') ? 'border-red-500' : 'border-gray-300' }}">
                                @error('kamar_mandi')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="luas_tanah" name="luas_tanah" value="{{ $properti->luas_tanah ?? old('luas_tanah') }}" min="0" class="w-full px-4 py-2 border dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded {{ $errors->has('luas_tanah') ? 'border-red-500' : 'border-gray-300' }}">
                            @error('luas_tanah')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="pemilik_id" name="pemilik_id" required class="w-full px-4 py-2 border dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded {{ $errors->has('pemilik_id') ? 'border-red-500' : 'border-gray-300' }}">
                                ...
                                @foreach($pemilik as $p)
                                    <option value="{{ $p->id }}" {{ $properti->pemilik_id == $p->id ? 'selected' : '' }}>{{ $p->username }}</option>
                                @endforeach
                            </select>
                            @error('pemilik_id')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
